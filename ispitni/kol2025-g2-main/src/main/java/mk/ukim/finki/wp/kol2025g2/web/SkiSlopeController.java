...
public class SkiSlopeController {
    ...
    public String listAll(@RequestParam(required = false) String name,
                          ...
                          Model model) {
        model.addAttribute("page",skiSlopeService.findPage(name,length,difficulty,skiResort,pageNum-1,pageSize));
    ...
    public String showEdit(@PathVariable  Long id, Model model) {
        SkiSlope skiSlope = skiSlopeService.findById(id);
        model.addAttribute("skiSlope",skiSlope);
        model.addAttribute("difficulties",SlopeDifficulty.values());
        model.addAttribute("skiResorts",skiResortService.listAll());
        return "form";
    }
